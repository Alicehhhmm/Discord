...
export const MembersModal = () => {
    ...
    const { server } = data as AsSrverType
    ...
    const onRoleChange = async (memberId: string, role: MemberRole) => {
        try {
            ...
            console.log('[MembersModal-onRoleChanged]', err)
        } finally {
            setLoadingId('')
        }
    }
    ...
    const onKick = async (memberId: string) => {
        try {
            ...
            console.log('ONKICK_ERROR', error)
        } finally {
            setLoadingId('')
        }
    }
                ...
                <ScrollArea className='mt-8 max-h-[420px] pr-6'>
                    {server?.members?.map(member => (
                        <div key={member.id} className='flex items-center gap-x-2 mb-6'>
                                                ...
                                                <DropdownMenuPortal>
                                                    <DropdownMenuSubContent>
                                                        <DropdownMenuItem onClick={() => onRoleChange(member.id, 'GUEST')}>
                                                            ...
                                                            <span>Guest</span>
                                                            {member.role === 'GUEST' && <Check className='w-4 h-4 mr-2' />}
                                                        </DropdownMenuItem>
                                                        <DropdownMenuItem onClick={() => onRoleChange(member.id, 'MODERATOR')}>
                                                            <Shield className='w-4 h-4 mr-2' />
                                                            <span>Moderator</span>
                                                            {member.role === 'MODERATOR' && <ShieldCheck className='w-4 h-4 mr-2' />}
                                                        </DropdownMenuItem>
                                                    </DropdownMenuSubContent>
                                                </DropdownMenuPortal>
                                            </DropdownMenuSub>
                            ...
                            {loadingId === member.id && <Loader2 className='w-4 h-4 text-zinc-500 animate-spin' />}
                        </div>
                    ))}
                </ScrollArea>
            </DialogContent>
